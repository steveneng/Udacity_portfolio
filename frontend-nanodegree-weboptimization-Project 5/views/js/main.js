...
var resizePizzas = function(size) {
  ...
  function changeSliderLabel(size) {
    var pick= document.getElementsByClassName("pizzaSize");
    switch(size) {
      case "1":
        pick.innerHTML = "Small";
        return;
      case "2":
        pick.innerHTML = "Medium";
        return;
      case "3":
        pick.innerHTML = "Large";
        return;
      default:
  ...
  var windowWidth = document.getElementById("randomPizzas").offsetWidth;// changed to document.getElementById() also changed window width to global variable
  ...
  function changePizzaSizes(size) {
    var Pcontainer= document.getElementsByClassName("randomPizzaContainer"); // changed querySelectorAll to document.getElementsByClassName()
    var PcontainerLength= Pcontainer.length; // changed array length into a variable
    var selector;
    for (var i = 0; i <PcontainerLength; i++) {
      selector= Pcontainer[i];
      var newwidth = (selector.offsetWidth + determineDx(selector, size)) + 'px';
      selector.style.width = newwidth;
    }
  }
...
var pizzasDiv = document.getElementById("randomPizzas"); // changed into getElementById()
for (var i = 2; i < 100; i++) {
...
function updatePositions() {
  ...
  var items = document.getElementsByClassName('mover');  // changed to getElementById from querySelectorAll
  var phase;
  var itemLength= items.length; // changed items.length into variable
  for (var i = 0; i < itemLength; i++) {
    phase = Math.sin(math + (i % 5));
...
var intViewportHeight = window.innerHeight*(.05);
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  var elem; // delcared elem as variable outside of forloop
  var movingPizza= document.querySelector("#movingPizzas1"); // took out q selector outside and changed it to a variable
  for (var i = 0; i < intViewportHeight; i++) {
